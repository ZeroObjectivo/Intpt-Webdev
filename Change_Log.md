...
### Fixed
- **Admin Warning Notification RLS:** Fixed a critical bug where administrators were blocked from sending "Warn Author" messages due to a "row violates row-level security policy" error. The RLS policy for inserting notifications was too restrictive. A new database migration (`20260514000900_fix_notification_insert_policy.sql`) has been added to authorize all variations of admin roles (e.g., `super admin`, `superadmin`), ensuring all admins can send warnings successfully.
