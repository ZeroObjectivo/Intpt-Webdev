...
- **Create Post Category Field Styling:** Aligned the Heron Business and Events dynamic form sections with the Create Post floating card surface by using a shared white `rounded-[12px]` container, clipped overflow, subtle shadow, Metropolis typography, and mobile-friendly event date fields. (References: `flask_backend/app/templates/dashboard.html`, `flask_backend/tests/test_dashboard_layout.py`)
- **Heron Business Filtering:** Fixed the dashboard Heron Business filter by aligning the frontend pill query value with the stored `Heron Business` category and adding backend alias support for legacy `Buy & Sell` category values. (References: `flask_backend/app/templates/dashboard.html`, `flask_backend/app/routes/core.py`, `flask_backend/app/static/js/modal.js`)
...
- **Facebook-style Image Grid:** Implemented a robust multi-image grid system for post cards (1-5+ images) with optimized layouts, aspect ratios, and modern aesthetic in `dashboard.css`.
- **Improved Image Sizing:** Fixed improper image stacking and sizing by using a CSS grid-based approach with `object-fit: cover` and variable heights for single images.
- **Template Cleanup:** Refactored `dashboard.html` and `profile_settings.html` to remove redundant image rounding and ensure compatibility with the new grid system.
