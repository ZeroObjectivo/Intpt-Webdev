...
- **Admin Moderation Refactor:** Integrated the post approval system into the general content management dashboard. Admins can now filter between "Recent", "Older", and "Pending" posts via a dropdown, streamlining the workflow and removing the redundant `/admin/approvals` page. (References: `flask_backend/app/routes/admin.py`, `flask_backend/app/templates/admin/content_manage.html`)
